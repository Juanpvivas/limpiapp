...
| III. Convenciones de Código y Estilo Dart | `NewReportScreen` es `ConsumerWidget` (lee `NewReportNotifier` vía `ref.watch`). `ConfirmationScreen` es `StatelessWidget`: no lee ningún provider, solo recibe el `Report` ya resuelto vía `extra` de navegación (Excepción del Principio III). `snake_case.dart`, `PascalCase` en clases, `const` obligatorio donde el compilador lo permita. El campo de texto de dirección manual usa un `TextEditingController` como estado efímero local (no de negocio), consistente con la excepción ya documentada para estado puramente local. | ✅ PASS |
