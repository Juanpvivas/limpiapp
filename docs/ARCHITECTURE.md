...
**Solo Repositories y Services tienen interfaz.** Los Notifiers, AsyncNotifiers y Use Cases son la lógica de negocio en sí misma — no hay una implementación alternativa que intercambiar, y ya son testeables inyectando un Repository fake. Darles también una interfaz sería pura ceremonia. El mismo razonamiento aplica a los wrappers de SDKs de terceros (cámara vía `image_picker`, ubicación vía `geolocator`/`geocoding`, mapa base vía `flutter_map` —con `latlong2` y `flutter_map_marker_cluster`—, conectividad vía `connectivity_plus`, notificaciones push, analytics, almacenamiento local): si la app habla con algo externo y variable, se envuelve detrás de una interfaz — ver el Principio I de la constitución para la lista completa de paquetes aprobados para cada capacidad. Los estados de error de las pantallas de lectura ("Mis reportes", "Mapa de reportes") comparten un único widget `DataErrorState` en `lib/ui/core/ui/`, y el aviso global de "sin conexión" se inserta en el `builder:` de `MaterialApp.router` (`lib/app.dart`).

> **Carve-out del Principio V (constitución v2.6.0)**: una interfaz de Repository/Use Case que expone un *flujo de estado observable* (ej. `ConnectivityRepository.watch() → Stream<ConnectivityStatus>`) puede devolver `Stream<T>` crudo, sin `Either`, si el error de la fuente se degrada a un valor por defecto de `T` dentro de Data. No aplica a operaciones puntuales falibles.
...
* **Excepción — renderizado del mapa (`flutter_map`)**: los fallos al descargar teselas/mosaicos, un proveedor de teselas inaccesible o la falta de conexión **no** se envuelven en un `Failure` (no provienen de un Repository/Service de datos); se manejan en la capa de Presentación degradando la vista (ej. mapa sin mosaicos pero con los marcadores y controles operativos), sin bloquear la pantalla. Los datos que alimentan los marcadores (ej. la lista de reportes) sí llegan por un Repository/stream y sus errores se mapean a `Failure` como cualquier otro dato.
...
* **Mapa base (features geográficas)**: `flutter_map` renderiza un mapa interactivo sobre teselas de OpenStreetMap (sin API key ni facturación de un proveedor de mapas); `latlong2` es su tipo de coordenadas y `flutter_map_marker_cluster` agrupa marcadores próximos según el zoom. La atribución de OpenStreetMap debe permanecer visible en pantalla. Los datos que se dibujan como marcadores (ej. reportes) provienen del mismo Repository/stream de Firestore — solo el fondo cartográfico es responsabilidad de `flutter_map` (ver sección 3 para el manejo de fallos de teselas).
...
6. **Sin excepciones más allá de Data**: toda excepción capturada en un Service/Repository debe mapearse a un `Failure`; un `throw` nunca debe cruzar hacia Domain/Presentation. Esto incluye las excepciones propias de Firebase (`FirebaseException` y subtipos) cuando la feature usa Firestore/Storage como backend, y las denegaciones de permiso o errores de `image_picker`, `flutter_image_compress`, `geolocator`, `geocoding` y `permission_handler` (ej. `PermissionFailure`, `LocationFailure`). **Salvedad**: los errores de renderizado de `flutter_map` (teselas que no cargan, proveedor de mapa caído, sin conexión) no se mapean a `Failure` — se degradan en Presentación (mapa sin mosaicos pero funcional), porque una tesela es un recurso de UI, no un dato de dominio.
